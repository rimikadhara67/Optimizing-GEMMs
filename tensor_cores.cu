...
__global__ void tensor_gemm_kernel(const half* __restrict__ d_A, 
...
void tensor_gemm(half* d_A, half* d_B, float* d_C, int M, int N, int K) {
    ...
    tensor_gemm_kernel<<<grid, block>>>(d_A, d_B, d_C, M, N, K);
}
...
int main() {
    ...
    if (!props.managedMemory) {
        std::cout << "Device does not support managed memory.\n";
        return -1;
    }
